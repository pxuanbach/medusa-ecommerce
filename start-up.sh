...
if [ "$REGION_COUNT" -gt 0 ]; then
  ...
  docker compose exec $BACKEND_SERVICE yarn seed
fi

# Step 4.1
echo "Build docker images"
docker compose build --no-cache $STOREFRONT_SERVICE

# Step 4.2
# Remove current storefront group
docker compose down $STOREFRONT_SERVICE

# Step 4.3
# Start all services in the storefront group
# $1 to pass more options into command
docker compose up -d $1 $STOREFRONT_SERVICE
